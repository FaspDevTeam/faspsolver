...
/*     
     ...
     WARNING: The subroutines contained in this file are modified from 
     SPARSEKIT 2.0 and they are NOT part of the FASP package.

     SPARSKIT is a package of FORTRAN subroutines for working with
     sparse matrices. It includes general sparse matrix manipulation
     routines as well as a few iterative solvers.

     SPARSKIT is free software; you can redistribute it and/or modify 
 ...
 \file ilu.f --- translated by Chunsheng Feng, Sep. 3, 2016
 ...
 *                    REAL *alu,INT *jlu,INT iwk,INT *ierr,INT *nzlu)
 *
 * \brief Get incomplete LU factorization with level of fill-in of k (ilu(k)) for a CSR matrix A  
 *
...
F995:
	printf("incomprehensible error. Matrix must be wrong.\n");
	*ierr = -1;
	goto F100;

    // insufficient storage in L.
F996:
	printf("insufficient storage in L.\n");
	*ierr = -2;
	goto F100;

    // insufficient storage in U.
F997:
	printf("insufficient storage in U.\n");
	*ierr = -3;
	goto F100;

    //  illegal lfil entered.
F998:
	printf("illegal lfil entered\n");
	*ierr = -4;
	return;

     // zero row encountered in A or U. 
F999:
	printf("zero row encountered in A or U.\n");
	*ierr = -5;
	goto F100;
/*----------------end-of-iluk--------------------------------------------
  ---------------------------------------------------------------------- */
 ...
 *
 * \brief Get incomplete LU factorization with dual truncation mechanism of a CSR matrix A  
 *
 * \param n   row number of A
 * \param a   nonzero entries of A
 * \param ja  integer array of column for A
...
F100:
	...
	//     incomprehensible error. Matrix must be wrong.
F995:
	printf("input matrix may be wrong \n");
	*ierr = -1;
    goto F100;
 ...
 *                    INT *ierr,INT *nz)
 *
 * \brief Get incomplete LU factorization with pivoting dual truncation mechanism of a CSR matrix A  
 *
 * \param n   row number of A
 * \param a   nonzero entries of A
 * \param ja  integer array of column for A
