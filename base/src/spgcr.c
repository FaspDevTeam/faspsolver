...
/**
 *	\fn void fasp_blas_dense_aAxpby(INT n, INT m, REAL *A, REAL alpha, REAL *x, REAL beta, REAL *y)
 *
                           ...
                           const SHORT print_level)
{
    ...
    while ( iter < MaxIt && sqrt(relres) > tol ) {
		...
		if (rst==0) fasp_blas_dense_aAxpby(n, i+1, z, 1.0, tmpx, 0.0, x->val);
		else fasp_blas_dense_aAxpby(n, i+1, z, 1.0, tmpx, 1.0, x->val);
	}
