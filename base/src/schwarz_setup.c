...
/**
 * \fn levels(int inroot, dCSRmat *A, int *mask, int *nlvl, int *iblock, int *jblock, int maxlev)
 *
...
/**
 * \fn fasp_schwarz_get_block_matrix(Schwarz_data *schwarz, int nblk, int *iblock, int *jblock, int *mask)
 *
...
{ 
	...
	/*-------------------------------------------*/
	// first pass
    // do a maxlev level sets out for each node  
	for ( i = 0; i < MIS.row; i++ ) {
        inroot = MIS.val[i];
    ...
    switch (block_solver) {
       ...
       /* use MUMPS direct solver on each block */
        case SOLVER_MUMPS: {
             ...
             for (i=0; i<nblk; ++i)  mumps[i] = fasp_mumps_factorize(&blk[i], NULL, NULL, PRINT_NONE);
             schwarz->mumps = mumps;
...
#endif
        /* nothing need to for iterative methods */
        default: {
        // nothing
		}
	}
    ...
    for (is=0; is<nblk; ++is) {
        ...
        switch (block_solver) {
            ...
            case SOLVER_UMFPACK: {
			 ...
			 fasp_solver_dcsr_pvgmres (&blk[is], &rhs, &u, NULL, 1e-8, 100, 20, 1, PRINT_NONE);
		 }
