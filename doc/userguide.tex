...
%
\item Line 24 reads solver parameters from ``tutorial/ini/amg.dat''. In this files, we can set the location of the data files, type of solvers, maximal number of iteration numbers, convergence tolerance, and many other parameters for iterative solvers. 
%
\item Line 32 defines a sparse matrix $A$ in the compressed sparse row (CSR) format. Line 33 defines two vectors: the right-hand side $b$ and the numerical solution $x$.
%
...
%
\item Line 58 solves $Ax=b$ using the AMG method. Type the AMG method and other parameters have been given in ``amgparam'' at Line 24. 
%
...
%
\item Line 57 solves $Ax=b$ using the AMG method. Type the AMG method and other parameters have been given in ``itparam''. 
%
\end{itemize}
%
...
\begin{itemize}
%
\item Line 7 sets the working directory, which contains the data files for matrices and right-hand side vectors. 
%
...
%
\item Line 14--25 gives the basic AMG parameters. For example, type of AMG, type of multilevel cycles, type of smoothers, number of maximal levels, etc. 
%
\item Line 31--35 gives the parameters for the setup phase of classical AMG methods.
%
\item Line 41--44 gives the parameters for the setup phase of aggregation-base AMG methods. 
%
\end{itemize}
%
Note that the input parameters allowed in FASP are not limited to the ones used in this example. For more parameters and their ranges, we refer to the Reference Manual. 
...
In this chapter, we discuss the basic data structures and a few important building blocks for constructing auxiliary space preconditioners for complicated PDE problems. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{More advanced usage}\label{ch:advance}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
