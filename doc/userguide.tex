...
interactions, Oldryod-B and Johnson-Seglman models, black-oil model, and magnetohydrodynamics (MHD).
...
and the header files in \verb|$(faspsolver)/include|.
...
Many options can be changed in "FASP.mk", as well
  as from the command line. For example
\begin{lstlisting}[numbers=none]
$ make config CC=gcc49 prefix=/usr/local
$ sudo make install
\end{lstlisting}
  will install in \verb|/usr/local/lib| and the include files in
  \verb|/usr/local/include| so that they may be accessed by multiple
  users.
...
\begin{snugshade}
  ...
  administrative privileges.  Rather, we rather recommend installation
  of FASP locally and then, if needed, copying them to a different
  location.
\end{snugshade}
...
\end{lstlisting}
%
If you use OpenMP very often and do not want type in this extra command-line option, you need to uncomment one line in ``FASP.mk'' and set ``openmp'' to be ``yes''.
